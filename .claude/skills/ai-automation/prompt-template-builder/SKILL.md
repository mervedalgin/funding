---
description: Create a prompt template builder with structured process, quality checks, and system integration
---
...
Eyleme dönüştürülebilir, ölçülebilir sonuçlar sunan kapsamlı bir prompt template builder oluşturun. Bu beceri, her seferinde profesyonel düzeyde çıktı sağlayan, kalite doğrulamalı yapılandırılmış bir süreç sunar.
...
- Projedeki mevcut prompt template builder belgelerini gözden geçir
- İlgili öğrenilmiş kurallar veya kısıtlamalar için `knowledge-base.md`'yi kontrol et
...
- prompt template builder yapılandırmak için seçilen çerçeveyi uygula
...
- prompt template builder aşağıdaki çıktı formatını kullanarak yapılandır
